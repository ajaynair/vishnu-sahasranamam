...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (audioRef.current) {
      ...
      const handleEnded = () => {
        setIsPlaying(false);
        setCurrentTime(0);
        // Optional: keep modal open vs close on end
      };
  ...
  useEffect(() => {
    ...
    const headerHeight = stickyControlsRef.current?.offsetHeight || 0;
    const targetScrollY = (sahasranamamTextRef.current?.offsetTop || 0) - headerHeight - 20; // 20px buffer

    window.scrollTo({
      top: Math.max(0, targetScrollY), // Ensure not scrolling to negative
      behavior: 'smooth'
    });
  }, [selectedLanguage]);
  ...
  const renderContent = (text: string, langId: string) => {
    ...
    return lines.map((line, index) => {
      ...
      const isNumericLine = /^\d+A?$/.test(trimmedLine.replace(/[॥।॥ ..\s]/g, '')); // Check if line is purely numeric (ignoring ., ॥, ।)
      const isSpecialStanza = /^\s*॥\s*\d+\s*॥\s*$/.test(trimmedLine); // e.g., ॥ 1 ॥

      // Highlight numerical markers or small phrases like "Pūrva Pīṭhikā" etc.
  ...
  const adjustFontSize = (amount: number) => {
    setFontSize(prevSize => {
      ...
      return Math.max(0.8, Math.min(2.5, newSize)); // Clamp between 0.8rem and 2.5rem
    });
  };
  ...
  const handleProgressChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (audioRef.current) {
      ...
      // audioRef.current.currentTime = newTime; // This is handled onMouseUp for seeking
    }
  };
  ...
  return (
      <div className="min-h-screen bg-gradient-to-br from-orange-100 via-amber-50 to-orange-100 flex flex-col">
        <header className="bg-orange-600 text-white shadow-md py-3 px-4 sm:px-6 sticky top-0 z-30">
          <div className="container mx-auto max-w-5xl flex flex-col sm:flex-row justify-between items-center">
            <div className="text-center sm:text-left mb-2 sm:mb-0">
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Vishnu Sahasranamam</h1>
              <p className="text-xs sm:text-sm text-orange-100">The Thousand Names of Vishnu - Multilingual</p>
            </div>
          </div>
        </header>

        <div ref={stickyControlsRef} className="sticky top-[68px] sm:top-[60px] z-20 bg-white/80 backdrop-blur-md shadow-sm rounded-b-lg">
        ...
        <main className="container mx-auto max-w-3xl p-4 sm:p-6 flex-grow">
          <div className="bg-white shadow-xl rounded-lg p-5 sm:p-8 ">
            <div
                ref={sahasranamamTextRef}
                className="font-serif text-gray-800 prose-sm sm:prose-base md:prose-lg max-w-none"
                style={{ fontSize: `${fontSize}rem` }}
                lang={selectedLanguage.id === 'english' ? 'en' : selectedLanguage.id} // Basic lang attribute for accessibility
            >
